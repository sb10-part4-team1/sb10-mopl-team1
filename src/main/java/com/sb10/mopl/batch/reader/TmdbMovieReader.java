...
/** TMDB 인기 영화(Movie) 데이터를 페이지 단위로 지연 로딩(Lazy Loading)하여 반환하는 페이징 리더입니다. */
@Slf4j
@Component
@StepScope
public class TmdbMovieReader implements ItemReader<TmdbContentDto> {
  ...
  public TmdbMovieReader(TmdbApiClient tmdbApiClient) {
  ...
  private void fetchNextPage() {
    ...
    log.info("TMDB 인기 영화 API 호출 - page: {}", page);

    TmdbApiResponse response = tmdbApiClient.fetchPopularMovies(page);
