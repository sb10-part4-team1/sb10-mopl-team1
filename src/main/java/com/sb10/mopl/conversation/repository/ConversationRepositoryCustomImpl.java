...
public class ConversationRepositoryCustomImpl implements ConversationRepositoryCustom {
  ...
  public List<Conversation> search(UUID myUserId, ConversationSearchRequest request) {
    ...
    return queryFactory
      ...
      .from(conversation)
      .join(meP).on(meP.conversation.eq(conversation))
      .join(otherP).on(otherP.conversation.eq(conversation))
   ...
   * <p>
   * 대화 목록의 정렬 기준은 생성일(createdAt) 하나뿐이므로, createdAt과 id를 조합한 복합 커서로 다음 페이지를 판별합니다.
   */
  private BooleanExpression cursorCondition(String cursor, UUID idAfter,
    ...
    try {
      ...
      return isAsc
        ? conversation.createdAt.gt(cursorTime)
        .or(conversation.createdAt.eq(cursorTime).and(conversation.id.gt(idAfter)))
        : conversation.createdAt.lt(cursorTime)
