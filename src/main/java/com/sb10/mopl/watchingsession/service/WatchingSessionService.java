...
public class WatchingSessionService {
  ...
  public WatchingSessionJoinResult join(UUID watcherId, UUID contentId) {
    ...
    if (existing.isPresent()) {
      WatchingSession watchingSession = existing.get();
      UUID previousContentId = watchingSession.getContent().getId();

      // 같은 콘텐츠에 이미 참여 중이면 기존 세션을 그대로 반환한다.
      ...
      watchingSession.updateContent(newContent);
      ...
      return new WatchingSessionJoinResult(
        watchingSessionMapper.toDto(watchingSession), previousDto);
    }
