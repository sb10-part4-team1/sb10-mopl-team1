...
class ReviewControllerTest {
  ...
  void create_returnCreated_whenRequestIsValid() throws Exception {
    ...
    ReviewCreateRequest request =
      new ReviewCreateRequest(contentId, "좋은 콘텐츠입니다.", 5);
  ...
  void create_returnNotFound_whenContentDoesNotExist() throws Exception {
    ...
    when(reviewService.create(request, CURRENT_USER_ID))
      .thenThrow(contentNotFoundException());
  ...
  void create_returnConflict_whenReviewAlreadyExists() throws Exception {
    ...
    when(reviewService.create(request, CURRENT_USER_ID))
      .thenThrow(reviewAlreadyExistsException());
    ...
    resultActions
      ...
      .andExpect(
        jsonPath("$.code")
          .value(ReviewErrorCode.REVIEW_ALREADY_EXISTS.getCode()));
  ...
  void findAll_returnBadRequest_whenSortByIsInvalid() throws Exception {
    ...
    resultActions
      ...
      .andExpect(
        jsonPath("$.code")
          .value(ReviewErrorCode.INVALID_REVIEW_VALUE.getCode()));
  ...
  void update_returnForbidden_whenUserIsNotAuthor() throws Exception {
    ...
    resultActions
      ...
      .andExpect(
        jsonPath("$.code")
          .value(ReviewErrorCode.UNAUTHORIZED_REVIEW_ACCESS.getCode()));
  ...
  void delete_returnNoContent_whenRequestIsValid() throws Exception {
    // when
    ResultActions resultActions =
      mockMvc.perform(delete(REVIEW_DETAIL_URL, reviewId));
  ...
  void delete_returnNotFound_whenReviewDoesNotExist() throws Exception {
    // given
    doThrow(reviewNotFoundException())
      .when(reviewService)
      .delete(reviewId, CURRENT_USER_ID);
  ...
  void delete_returnForbidden_whenUserIsNotAuthor() throws Exception {
    // given
    doThrow(unauthorizedReviewException())
      .when(reviewService)
      .delete(reviewId, CURRENT_USER_ID);
  ...
  private ReviewException invalidSortByException() {
    return new ReviewException(
      ...
      Map.of("sortBy", "지원하지 않는 정렬 기준입니다."));
  }
